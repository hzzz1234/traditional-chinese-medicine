    ...
    _init: function(el, options){
      ...
      $.extend(true, this.options, options);
        ...
        _timeTmplData: function(){
            ...
            return '<div class="rangslider"><div class="rangslider-show"><span class="label label-info calendar-showtime">'+targets.substr(1)+'</span></div><ul class="unstyled rangslider-handle">'+ids+'</ul></div><div class="calendar-ctrl"><button data-type="submit" class="btn btn-primary btn-block">纭畾</button></div>';
        },
...
;(function($){
            ...
            if( that.put ){
                ...
                if( that.cid != 'init' ){
                    if( that.usechannel && that.cname ){
                        h += '<a style="padding-left:10px;border-bottom: solid 1px #eee;line-height: 42px;display:block;" href="javascript:;" data-pchannel="'+that.cname+'">鐐规閫夋嫨 '+that.cname+' 棰戦亾涓嬬殑鎵€鏈夐〉闈�</a>'
                    }
                    h += '<ul class="unstyled">'
                    for (var i = 0; i < m.length; i++) {
  ...
  querypage.prototype = {
        ...
        _init: function(el, options){
          ...
          $.extend(true, this.options, options);
            ...
            el.bind('click', function(e){
                ...
                $(that.options.container).append('<div class="querypage" id="'+uid+'" style="display:none;"><div class="querypage-search"><input type="text" class="input-block-level querypage-input" placeholder="璇疯緭鍏ラ〉闈D鎴栭〉闈㈠悕绉�"></div><div class="querypage-content"><ul class="querypage-channel"></ul><div class="querypage-page"></div></div></div>');
                that._target = $('#'+uid);
        ...
        _change: function(){
            var that = this;
            that._keyword = $.trim( encodeURI(that._input.val().replace(/[',.锛屻€傦紒*--+`~!@#$%^&*]/g, '')) )
            ...
            if( that._keyword.length ){
                if( that._keyword.length < 2 ){
                    ...
                    that.hem.p.html( '<p class="pagenull">璇疯緭鍏ヨ嚦灏�2涓瓧绗�</p>' )
                }else{
                    that.hem.put = true
